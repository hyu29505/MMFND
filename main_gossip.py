...
parser.add_argument('--model_name', default='domain_gossipcop', help="要使用的模型架构名称。目前gossipcop使用'domain_gossipcop', weibo/weibo21使用'domain_weibo'")
parser.add_argument('--dataset', default='gossipcop', choices=['gossipcop', 'weibo', 'weibo21'], help="要使用的数据集。")
...
parser.add_argument('--use_hier_modules', action='store_true', help="启用一致性与层级融合模块。")
parser.add_argument('--use_cross_modal_calibration', action='store_true', help="启用跨模态校准模块。")
parser.add_argument('--use_hier_consistency', action='store_true', help="启用一致性验证模块。")
parser.add_argument('--use_hier_fusion', action='store_true', help="启用层级融合噪声过滤模块。")
parser.add_argument('--use_calibration_loss', action='store_true', help="启用校准损失。")
parser.add_argument('--consistency_weight', type=float, default=0.0, help="一致性损失权重。")
parser.add_argument('--calibration_weight', type=float, default=0.0, help="校准损失权重。")
parser.add_argument('--calibration_temperature', type=float, default=0.5, help="校准温度。")
parser.add_argument('--loss_type', default='bce', choices=['bce', 'focal'], help="损失函数类型：bce 或 focal。")
parser.add_argument('--focal_gamma', type=float, default=2.0, help="Focal Loss 的 gamma。")
parser.add_argument('--focal_alpha', type=float, default=0.25, help="Focal Loss 的 alpha (正类权重)。")
parser.add_argument('--eval_threshold', type=float, default=0.5, help="评估时的概率阈值。")
parser.add_argument('--tune_threshold', action='store_true', help="在验证集上搜索最优阈值（默认最大化 F1）。")
parser.add_argument('--tune_threshold_metric', default='F1', choices=['acc', 'F1'], help="阈值搜索的优化指标（acc 或 F1）。")
parser.add_argument('--threshold_min', type=float, default=0.1, help="阈值搜索的最小值。")
parser.add_argument('--threshold_max', type=float, default=0.9, help="阈值搜索的最大值。")
parser.add_argument('--threshold_step', type=float, default=0.05, help="阈值搜索的步长。")
...
config = {
    ...
    'model_name': current_model_name, # 使用调整后的模型名称
        ...
        'use_hier_modules': args.use_hier_modules,
        'use_cross_modal_calibration': args.use_cross_modal_calibration,
        'use_hier_consistency': args.use_hier_consistency,
        'use_hier_fusion': args.use_hier_fusion,
        'use_calibration_loss': args.use_calibration_loss,
        ...
        'tune_threshold_metric': args.tune_threshold_metric,
