...
try:
    ...
    from mm.utils.utils_weibo import data2gpu, Averager, metrics, Recorder, clipdata2gpu
    from mm.utils.utils_weibo import metricsTrueFalse
from .layers import *
from .pivot import *
...
class MultiDomainPLEFENDModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        return_calibration_features = kwargs.get('return_calibration_features', False)
        ...
        clip_image_feature = clip_image_feature.float()
        clip_text_feature = clip_text_feature.float()
        ...
        if self.enable_cross_modal_calibration:
            text_feature, image_feature = self.cross_modal_calibration(
                text_feature,
                image_feature,
                clip_text_feature,
                clip_image_feature,
        ...
        if self.enable_calibration_loss:
            if masks is None:
                text_pooled = text_feature.mean(dim=1)
            else:
                text_mask = masks.unsqueeze(-1)
                text_pooled = (text_feature * text_mask).sum(dim=1) / text_mask.sum(dim=1).clamp_min(1.0)
            image_pooled = image_feature.mean(dim=1)
        ...
        if self.enable_hier_fusion:
            if s_l is None or s_g is None:
                s_l = torch.zeros(text_feature.size(0), device=text_feature.device)
                s_g = torch.zeros(text_feature.size(0), device=text_feature.device)
        ...
        if hier_fusion_feature is not None:
            fusion_features = fusion_features + hier_fusion_feature
        ...
        if hier_fusion_feature is not None:
            all_modility = all_modility + hier_fusion_feature
        ...
        fake_news_sigmoid = _sanitize_probs(fake_news_sigmoid)
        text_fake_news = _sanitize_probs(text_fake_news)
        image_fake_news = _sanitize_probs(image_fake_news)
        fusion_fake_news = _sanitize_probs(fusion_fake_news)

        return fake_news_sigmoid, text_fake_news, image_fake_news, fusion_fake_news, cons_loss, cal_loss, consistency_scores
...
class DOMAINTrainerWeibo():
    def __init__(self,
        ...
        self.last_checkpoint_path = os.path.join(self.save_param_dir, 'last_checkpoint.pt')
    ...
    def train(self):
        ...
        if self.resume_enabled:
            resume_path = self.resume_path or (self.last_checkpoint_path if os.path.exists(self.last_checkpoint_path) else None)
        if resume_path is not None and os.path.exists(resume_path):
            checkpoint = torch.load(resume_path, map_location='cuda' if self.use_cuda else 'cpu')
            missing, unexpected = self.model.load_state_dict(
                checkpoint.get('model_state', checkpoint),
                strict=False
            )
            if missing:
                print(f"[Resume] Missing keys (likely new modules): {missing}")
            if unexpected:
            ...
            if 'scheduler_state' in checkpoint and scheduler is not None:
                try:
                    ...
                    print(f"[Resume] Skip scheduler state load: {e}")
            start_epoch = checkpoint.get('epoch', 0) + 1
            print(f"[Resume] Loaded checkpoint from {resume_path}, start_epoch={start_epoch}")
        for epoch in range(self.epoches):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                label0, text_fake_news, image_fake_news, fusion_fake_news, cons_loss, cal_loss, _ = self.model(**batch_data)
            ...
            if mark == 'save':
                ...
                continue
        final_ckpt_path = os.path.join(self.save_param_dir, 'parameter_clip111.pkl')
        if os.path.exists(final_ckpt_path):
            self.model.load_state_dict(torch.load(final_ckpt_path), strict=False)
        print("开始进行最后的测试: ")
        results0 = self.test(self.test_loader)
    ...
    @staticmethod
    def _hierarchical_consistency_loss(s_l, s_g, labels):
        labels = labels.float()
        real_loss = (1.0 - labels) * ((1.0 - s_l) ** 2 + (1.0 - s_g) ** 2)
        fake_loss = labels * torch.min(s_l ** 2, s_g ** 2)
        return (real_loss + fake_loss).mean()

    @staticmethod
    def _cross_modal_calibration_loss(text_calib, image_calib, tau=1.0):
        tau = max(float(tau), 1e-6)
        text_log_probs = F.log_softmax(text_calib / tau, dim=-1)
        image_probs = F.softmax(image_calib / tau, dim=-1)
        return F.kl_div(text_log_probs, image_probs, reduction='batchmean')
