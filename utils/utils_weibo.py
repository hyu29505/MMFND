...
def metrics(y_true, y_pred, category, category_dict):
    ...
    for c, res in res_by_category.items():
        try:
            auc_val = roc_auc_score(res['y_true'], res['y_pred'])
            auc_val = round(float(auc_val), 4)
        except ValueError:
            auc_val = 0
        metrics_by_category[c] = {'auc': auc_val}

    try:
        metrics_by_category['auc'] = round(float(roc_auc_score(y_true, y_pred, average='macro')), 4)
    except ValueError:
    ...
    for c, res in res_by_category.items():
        ...
        metrics_by_category[c] = {
            ...
            'fscore': f1_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int),
                               average='macro'),
            'auc': metrics_by_category[c].get('auc', 0),
